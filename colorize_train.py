...
SAVE_PATH = ROOT_PATH + '/Documents/Quad-S-Learning/save/colorize/'
...
parser.add_argument( "--savename",  type=str, default='erfnet_colorize_citywaug.pth')
...
def run_train_epoch(epoch,model,criterion,optimizer,data_loader):
    ...
    for step, batch_data in enumerate(data_loader):
        img_gray, img_ab, img = batch_data    
        img_gray, img_ab = Variable(img_gray).cuda(), Variable(img_ab).cuda()
        #Forward Propagation
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if val_avg_loss < best_val_avg_loss or epoch == 0:
