from data import fulljigsawLoader2 as dataset
...
parser.add_argument( "--savename",  type=str, default='erfnet_jigsaw_FS_citynoaug.pth')
...
def main():
    train_set = dataset(root_dir=DATASET_DIR, mode='train',dataset=args.dataset,include_extralabel=True)
    train_loader = data.DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=args.workers)
    val_set = dataset(root_dir=DATASET_DIR, mode='val',dataset=args.dataset,include_extralabel=True)
    ...
    model = ERFNet(num_classes=1000, encoder=pretrainedEnc, classify=True).cuda()
