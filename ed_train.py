...
# python ed_train.py --dataset cityscapes --encoder imagenet --pretrain-enc-name erfnet_class_imgnet.pth --pretrain-dec-name erfnet_encoder_otsuimg.pth --batch-size 6
# python ed_train.py --encoder rot --pretrain-enc-name erfnet_encoder_rot_city_waug.pth --decoder otsu --pretrain-dec-name erfnet_encoder_otsuimg.pth
...
parser.add_argument( "--name",           type=str, default='erfnet_ed_rotcitywaug_colorizecitynoaug.pth')
...
def main():
    ...
    if args.decoder == 'none':
        pretrainedDec = None
        print('Decoder is random init')
    elif args.decoder == 'otsu':
    ...
    elif args.decoder == 'colorize':
        print("Loading decoder pretrained on colorize")
        pretrained = ERFNet(2, classify=False)
        checkpnt = torch.load(ROOT_PATH+'/Documents/Quad-S-Learning/save/colorize/' + args.pretrain_dec_name)
        print("Loading decoder: ",args.pretrain_dec_name)
        pretrained.load_state_dict(checkpnt['state_dict'])
        pretrainedDec = pretrained.decoder
        pretrainedDec.output_conv = torch.nn.ConvTranspose2d( 16, num_classes, 2, stride=2, padding=0, output_padding=0, bias=True)
    ...
    for epoch in range(args.epochs):
        print('Train LR: ', lr_updater.get_lr())
