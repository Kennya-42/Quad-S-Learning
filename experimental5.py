...
parser.add_argument( "--encoder2",           choices=['none', 'class','colorize'], default='class')
...
parser.add_argument( "--pretrain-enc-name2",  type=str, default='erfnet_class_imgnet.pth')
parser.add_argument( "--pretrain-dec-name",  type=str, default='erfnet_otsu_citywaug.pth')
parser.add_argument( "--name",           type=str, default='erfnet_exp5.pth')
args = parser.parse_args()

def random_init_low_mag_featuremaps(model,model2):
    print('frankensteining feature maps')
    model_weights = []
    model_bias = []
            ...
            for subblock in block.children():#non-bt-1d and downsamplers
                for layer in subblock.children():#actual conv2d and batchnorm
                    if isinstance(layer, nn.Conv2d):
                        model_weights.append(layer.weight.data)
                        model_bias.append(layer.bias.data)
        ...
        if isinstance(block, DownsamplerBlock):#get initial block
            ...
            for name,subblock in block.named_children():#non-bt-1d and downsamplers
                ...
                for layer in subblock.children():#actual conv2d and batchnorm
                    ...
                    if isinstance(layer, nn.Conv2d):
                        if i > 68:
                            layer.weight.data = model_weights[j]
                            layer.bias.data = model_bias[j]
                        j+=1
...
def main():
    ...
    if not args.resume:
        ...
        if args.encoder2 == 'class':
            print("Loading encoder2 pretrained on imagenet classification")
            pretrained = ERFNet(1000,classify=True)
            ...
            print("Loading encoder2 pretrained on rotations")
            pretrained = ERFNet(4,classify=True)
            checkpnt = torch.load(ROOT_PATH+'/Documents/Quad-S-Learning/save/rotation/' + args.pretrain_enc_name2)
            print("Loading encoder: ", args.pretrain_enc_name2)
            pretrained.load_state_dict(checkpnt['state_dict'])
            pretrainedEnc2 = pretrained.encoder

        if args.decoder == 'otsu':
            print("Loading decoder pretrained on otsu cluistering")
            pretrained = ERFNet(4, classify=False)
            checkpnt = torch.load(ROOT_PATH+'/Documents/Quad-S-Learning/save/cluster/otsu/' + args.pretrain_dec_name)
            print("Loading decoder: ",args.pretrain_dec_name)
            pretrained.load_state_dict(checkpnt['state_dict'])
            pretrainedDec = pretrained.decoder
            pretrainedDec.output_conv = torch.nn.ConvTranspose2d( 16, num_classes, 2, stride=2, padding=0, output_padding=0, bias=True)

        random_init_low_mag_featuremaps(pretrainedEnc,pretrainedEnc2)
        # exit()
    else:
        pretrainedEnc = None
        pretrainedDec = None
    ...
    #Setup Model
    model = ERFNet(num_classes=num_classes, encoder=pretrainedEnc, decoder=pretrainedDec).cuda()
