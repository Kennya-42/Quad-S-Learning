...
parser.add_argument( "--name",           type=str, default='erfnet_exp_ed_lotto.pth')
...
def main():
    ...
    if not args.resume:
        ...
        checkpnt = torch.load(ROOT_PATH+'/Documents/Quad-S-Learning/save/cluster/otsu/' + "erfnet_encoder_otsuimg.pth")
        print("Loading decoder: erfnet_encoder_otsuimg.pth")
        pretrained.load_state_dict(checkpnt['state_dict'])
    ...
    print('pruning both encoder and decoder...')
    mask_list = []
    for param in model.parameters():
        if len(param.size()) > 1 :
            if param.size()[1] == 20:
                mask =  mask = torch.zeros(param.data.size(),dtype=torch.bool).cuda()
                mask_list.append(mask)
            else:
                mask = zero_param(param).cuda()
                mask_list.append(mask)
